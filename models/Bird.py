...
class Bird:
    ...
    def draw(self, window, dt):
        ...
        if self.is_jumping:
            self.y -= self.velocity * dt  # move the bird up
            # decrease velocity when bird gets higher
            self.velocity -= self.GRAVITY * dt  # it's basically a parabola, +jump_velocity -> -jump_velocity
            ...
            self.y -= self.velocity * dt  # subtract velocity from y coordinate to make the bird fall
            self.velocity -= self.GRAVITY * dt  # decrease velocity by gravity constant
